...
# Usage:
#   1. Open Terminal
#   2. Navigate to your React project root: cd /path/to/your/react-project
#   3. Run this script: bash setup_banking_surrogate.sh
#      Or: chmod +x setup_banking_surrogate.sh && ./setup_banking_surrogate.sh
#
...
echo ""
echo -e "${HEADER}=============================================${NC}"
echo -e "${HEADER}  Banking Surrogate - React Project Setup    ${NC}"
echo -e "${HEADER}  Mahajan Finance                             ${NC}"
echo -e "${HEADER}=============================================${NC}"
echo ""

# Check if we're in a React project
if [ ! -f "package.json" ]; then
    echo -e "${ERROR}[ERROR] No package.json found in the current directory.${NC}"
    echo -e "${ERROR}        Please run this script from your React project root.${NC}"
    exit 1
fi

# Check if src directory exists
if [ ! -d "src" ]; then
    echo -e "${ERROR}[ERROR] No 'src' directory found. This doesn't look like a standard React project.${NC}"
    exit 1
fi

echo -e "${SUCCESS}[OK] React project detected (package.json + src/ found)${NC}"
echo ""

# ─── Step 1: Create Directory Structure ───────────────────────────────────────

echo -e "${INFO}Step 1: Creating directory structure...${NC}"

for dir in src/lib src/components; do
    if [ ! -d "$dir" ]; then
        mkdir -p "$dir"
        echo -e "  ${SUCCESS}Created: $dir/${NC}"
    else
        echo -e "  ${WARNING}Already exists: $dir/${NC}"
    fi
done
...
echo -e "${INFO}Step 2: Writing Banking Surrogate source files...${NC}"
echo ""
...
echo -e "  ${INFO}Writing: src/lib/bankingTypes.ts${NC}"

cat > src/lib/bankingTypes.ts << 'BANKINGTYPES_EOF'
/**
...
echo -e "    ${SUCCESS}Done (bankingTypes.ts)${NC}"

# ──────────────────────────────────────────────────────────────────────────────
# File 2: src/lib/parseBankClient.ts
# ──────────────────────────────────────────────────────────────────────────────

echo -e "  ${INFO}Writing: src/lib/parseBankClient.ts${NC}"

cat > src/lib/parseBankClient.ts << 'PARSEBANKCLIENT_EOF'
/**
...
echo -e "    ${SUCCESS}Done (parseBankClient.ts)${NC}"

# ──────────────────────────────────────────────────────────────────────────────
# File 3: src/lib/generateBankPdf.ts
# ──────────────────────────────────────────────────────────────────────────────

echo -e "  ${INFO}Writing: src/lib/generateBankPdf.ts${NC}"

cat > src/lib/generateBankPdf.ts << 'GENERATEBANKPDF_EOF'
/**
...
echo -e "    ${SUCCESS}Done (generateBankPdf.ts)${NC}"
...
echo -e "  ${INFO}Writing: src/components/BankingSurrogate.tsx (React-adapted)${NC}"

cat > src/components/BankingSurrogate.tsx << 'BANKINGSURROGATE_EOF'
...
BANKINGSURROGATE_EOF

echo -e "    ${SUCCESS}Done (BankingSurrogate.tsx - React adapted)${NC}"

echo ""

# ─── Step 3: Print npm install commands ───────────────────────────────────────

echo -e "${INFO}Step 3: Install required npm packages${NC}"
echo ""
echo -e "  ${WARNING}The Banking Surrogate component requires the following npm packages:${NC}"
echo ""
echo -e "    ${INFO}npm install pdfjs-dist jspdf recharts${NC}"
echo ""
echo -e "  ${WARNING}Or if you use yarn:${NC}"
echo ""
echo -e "    ${INFO}yarn add pdfjs-dist jspdf recharts${NC}"
echo ""
echo -e "  ${WARNING}Or if you use pnpm:${NC}"
echo ""
echo -e "    ${INFO}pnpm add pdfjs-dist jspdf recharts${NC}"
echo ""
echo -e "  ${WARNING}Package details:${NC}"
echo -e "    ${INFO}- pdfjs-dist  : PDF parsing (reads bank statement PDFs, supports passwords)${NC}"
echo -e "    ${INFO}- jspdf       : PDF generation (creates professional banking analysis reports)${NC}"
echo -e "    ${INFO}- recharts    : Charting library (cash flow trend, credit vs debit charts)${NC}"
echo ""

# ─── Step 4: Print usage instructions ─────────────────────────────────────────

echo -e "${INFO}Step 4: Usage in your React app${NC}"
echo ""
echo -e "  ${WARNING}Import and use the component in your app:${NC}"
echo ""
echo -e '    import BankingSurrogate from "./components/BankingSurrogate";'
echo ""
echo -e "  ${WARNING}Then in your JSX:${NC}"
echo ""
echo -e "    <BankingSurrogate />"
echo ""
...
echo -e "${SUCCESS}=============================================${NC}"
echo ""
echo -e "  ${SUCCESS}Files created:${NC}"
echo -e "    ${SUCCESS}[1] src/lib/bankingTypes.ts       - Type definitions & currency formatter${NC}"
echo -e "    ${SUCCESS}[2] src/lib/parseBankClient.ts    - Bank statement parser & risk/EMI calculator${NC}"
echo -e "    ${SUCCESS}[3] src/lib/generateBankPdf.ts    - Professional PDF report generator${NC}"
echo -e "    ${SUCCESS}[4] src/components/BankingSurrogate.tsx - Main React component (adapted for Vite/CRA)${NC}"
echo ""
echo -e "  ${WARNING}Next steps:${NC}"
echo -e "    ${INFO}1. Run: npm install pdfjs-dist jspdf recharts${NC}"
echo -e "    ${INFO}2. Import BankingSurrogate in your app${NC}"
echo -e "    ${INFO}3. Make sure Tailwind CSS is configured in your project${NC}"
echo ""
echo -e "  ${WARNING}Note: This component was adapted from Next.js to standard React.${NC}"
echo -e "        ${WARNING}- Removed 'use client' directive${NC}"
echo -e "        ${WARNING}- Removed unused useEffect import${NC}"
echo -e "        ${WARNING}- All other functionality is unchanged${NC}"
echo ""
echo -e "  ${HEADER}Mahajan Finance | Sandeep Mahajan | +91 9730540215${NC}"
echo ""
