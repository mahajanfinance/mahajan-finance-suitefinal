# === SCRIPT 6: Professional UI for TabParties with Bulk Add ===

$projectRoot = $PSScriptRoot
if (-not $projectRoot) { $projectRoot = Get-Location }
$componentPath = Join-Path $projectRoot "src\components\tracker\TabParties.tsx"

$lines = @(
'import { useEffect, useState } from "react";',
'import { acc, fmtINR } from "@/lib/accounting";',
'import { toast } from "sonner";',
'import { Plus, Trash2, Users, Upload, X, Search } from "lucide-react";',
'',
...
'        parsed.push({ name: cols[0], type: cols[1] || "customer", gstin: cols[2] || "", phone: cols[3] || "", email: cols[4] || "", balance: Number(cols[5]) || 0 });',
'      }',
'    }',
...
'      user_id: userId, name: b.name.toUpperCase(), party_type: b.type,',
'      gstin: b.gstin.toUpperCase(), phone: b.phone, email: b.email,',
'      address: "", opening_balance: b.balance,',
'    }));',
...
'  };',
'',
'  const filtered = list.filter(p => !search || p.name.toLowerCase().includes(search.toLowerCase()) || (p.phone || "").includes(search));',
'  const customerCount = list.filter(p => p.party_type === "customer" || p.party_type === "both").length;',
'  const vendorCount = list.filter(p => p.party_type === "vendor" || p.party_type === "both").length;',
'',
'  return (',
...
'            <select value={form.party_type} onChange={e => setForm(p => ({ ...p, party_type: e.target.value }))} className={inputClass}>',
...
'          </div>',
'          <textarea value={form.address} onChange={e => setForm(p => ({ ...p, address: e.target.value }))} placeholder="Address" className={inputClass} rows={2} />',
...
'              <div><h4 className="font-extrabold text-violet-900">Bulk Party Import</h4><p className="text-xs text-violet-700">Tab-separated: Name, Type, GSTIN, Phone, Email, Balance</p></div>',
'            </div>',
'            <button onClick={() => { setBulkMode(false); setBulkText(""); setBulkPreview([]); }} className="p-1 rounded hover:bg-violet-200"><X size={16} /></button>',
'          </div>',
'          <textarea value={bulkText} onChange={e => setBulkText(e.target.value)} placeholder={`RAM KUMAR\\tcustomer\\t22AAAAA0000A1Z5\\t9876543210\\tram@email.com\\t5000\nSITA DEVI\\tvendor\\t\\t9876543211\nAMIT TRADERS\\tboth\\t\\t9876543212\\tamit@traders.com\\t10000`} rows={4} className={`${inputClass} font-mono text-xs`} />',
...
'                <th className="px-3 py-2 text-left">Name</th><th className="px-3 py-2">Type</th>',
'                <th className="px-3 py-2">Phone</th><th className="px-3 py-2 text-right">Balance</th>',
'              '</tr></thead><tbody>',
'                {bulkPreview.map((b, i) => <tr key={i} className="border-b"><td className="px-3 py-1.5 font-medium">{b.name}</td><td className="px-3 py-1.5 text-center">{b.type}</td><td className="px-3 py-1.5 text-center">{b.phone || "-"}</td><td className="px-3 py-1.5 text-right font-bold">{fmtINR(b.balance)}</td></tr>)}',
'              '</tbody></table>',
'            </div>',
'          )}',
'        </div>',
...
'            <div className="w-16 h-16 rounded-full bg-muted/50 flex items-center justify-center mx-auto mb-3"><Users size={28} className="text-muted-foreground" /></div>',
'            <p className="text-muted-foreground text-sm">No parties yet.</p>',
'            <p className="text-muted-foreground text-xs mt-1">Add your first customer or vendor</p>',
'          </div>',
'        ) : (',
...
'                      <span className={`text-xs px-2 py-0.5 rounded-full font-bold ${p.party_type === "customer" ? "bg-blue-100 text-blue-700" : p.party_type === "vendor" ? "bg-orange-100 text-orange-700" : "bg-purple-100 text-purple-700"}`}>{p.party_type}</span>',
'                    </td>',
...
'                      <button onClick={() => remove(p.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-red-500 transition-colors"><Trash2 size={14} /></button>',
'                    </td>',
'                  </tr>',
'                ))}',
'              '</tbody>',
...
Write-Host "=== TabParties.tsx UPDATED ===" -ForegroundColor Green
Write-Host "  + Bulk import (tab-separated)"
Write-Host "  + Search bar"
Write-Host "  + Customer/Vendor count"
Write-Host "  + Professional UI (empty state, badges, gradients)"