# ============================================================
# Mahajan Finance - Banking Surrogate V2.0 Setup
# ============================================================
# Run from: D:\mahajan-finance-suite-main cashflow added
# Usage:    powershell -ExecutionPolicy Bypass -File .\setup_v2.ps1
# ============================================================
...
if (Test-Path "src\lib\parseBankClient.ts") {
    New-Item -ItemType Directory -Path $backupDir -Force | Out-Null
    Copy-Item "src\lib\parseBankClient.ts" "$backupDir\parseBankClient.ts.bak" -ErrorAction SilentlyContinue
    Copy-Item "src\lib\generateBankPdf.ts" "$backupDir\generateBankPdf.ts.bak" -ErrorAction SilentlyContinue
    Copy-Item "src\pages\BankingSurrogate.tsx" "$backupDir\BankingSurrogate.tsx.bak" -ErrorAction SilentlyContinue
    Write-Host "  Backed up existing files to $backupDir\" -ForegroundColor DarkGray
}

Write-Host ""
Write-Host "Creating 4 files..." -ForegroundColor Yellow
Write-Host ""

# ============================================================
# FILE 1: src\lib\bankingTypes.ts (NEW)
# ============================================================
Write-Host "  [1/4] src\lib\bankingTypes.ts (NEW)" -ForegroundColor White
...
Set-Content -Path "src\lib\bankingTypes.ts" -Value $typesContent -Encoding UTF8
Write-Host "        Created" -ForegroundColor Green

# ============================================================
# FILE 2: src\lib\parseBankClient.ts (REPLACE)
# ============================================================
Write-Host "  [2/4] src\lib\parseBankClient.ts (REPLACE)" -ForegroundColor White
...
import type {
  ClientParseResult, MonthData, DayBalance, Transaction, ParseDiagnostics,
} from "./bankingTypes";
...
function parseDate(str: string): Date | null {
  ...
  if (m) { const d=parseInt(m[1]), mo=parseInt(m[2])-1; let y=parseInt(m[3]); if(y<100) y+=2000; return new Date(y,mo,d); }
  m = s.match(/^(\d{1,2})[\/\-\s]+([A-Za-z]{3,9})[\/\-\s]+(\d{2,4})$/i);
  if (m) { const d=parseInt(m[1]); const moIdx=MONTH_NAMES.findIndex(n=>m[2].toLowerCase().startsWith(n.toLowerCase())); let y=parseInt(m[3]); if(y<100) y+=2000; if(moIdx>=0) return new Date(y,moIdx,d); }
...
function detectBankName(text: string): string {
  ...
  const bankPatterns: [string, string][] = [
    ["SBI|STATE BANK OF INDIA","State Bank of India"],["HDFC BANK|HDFC","HDFC Bank"],
    ["ICICI BANK|ICICI","ICICI Bank"],["AXIS BANK","Axis Bank"],
    ["KOTAK MAHINDRA|KOTAK","Kotak Mahindra Bank"],["PUNJAB NATIONAL BANK|PNB","Punjab National Bank"],
    ["BANK OF BARODA|BOB","Bank of Baroda"],["CANARA BANK","Canara Bank"],
    ["UNION BANK OF INDIA","Union Bank of India"],["INDIAN BANK","Indian Bank"],
    ["BANK OF INDIA","Bank of India"],["IDFC FIRST BANK|IDFC","IDFC FIRST Bank"],
    ["INDUSIND BANK","IndusInd Bank"],["YES BANK","YES Bank"],
    ["FEDERAL BANK","Federal Bank"],["RBL BANK","RBL Bank"],
    ["BANDHAN BANK","Bandhan Bank"],["UCO BANK","UCO Bank"],
    ["CENTRAL BANK OF INDIA","Central Bank of India"],["BANK OF MAHARASHTRA","Bank of Maharashtra"],
    ["AU SMALL FINANCE","AU Small Finance Bank"],["INDIA POST PAYMENTS BANK|IPPB","India Post Payments Bank"],
  ];
  for (const [pattern, name] of bankPatterns) { if (new RegExp(pattern).test(upper)) return name; }
  return "Unknown Bank";
}
...
export function parseBankStatementClient(text: string): ClientParseResult {
  ...
  for (const line of lines) {
    for (const pat of rowPatterns) {
      ...
      if (match) {
        ...
        if (match[5]) {
          const val3=parseAmount(match[3]), val4=parseAmount(match[4]);
          balance=parseAmount(match[5]);
          const isDr=(match[6]||"").toUpperCase()==="DR";
          if(val3>0&&val4===0){if(/credit|cr|received|refund|cash\s*dep/i.test(narration))credit=val3;else debit=val3;}
          else if(val3>0&&val4>0){if(/credit|cr|received|refund/i.test(narration))credit=val4;else{debit=val3;credit=val4;}}
  ...
  for (const mk of sortedMonthKeys) {
    ...
    for (const targetDay of adjustedSampleDays) {
      let bestTxn: Transaction | null = null, bestDiff = Infinity;
    ...
    const monthlyABB = validDayBalances.length>0 ? validDayBalances.reduce((s,d)=>s+d.balance,0)/validDayBalances.length : 0;
    ...
    monthsData.push({
      monthLabel:`${MONTH_NAMES[month-1]} ${year}`,year,month,dayBalances,sampleDays:adjustedSampleDays,
      monthlyABB,openingBalance,closingBalance,totalCredits,totalDebits,creditCount,debitCount,
      highestBalance,lowestBalance,transactions:monthTxns,chequeBounces,emiBounces,
    });
  }

  const monthsWithABB = monthsData.filter(m => m.monthlyABB > 0);
  const overallABB = monthsWithABB.length>0 ? monthsWithABB.reduce((s,m)=>s+m.monthlyABB,0)/monthsWithABB.length : 0;
  const detectedPeriod = monthsData.length;
  const sortedDates = Array.from(dateSet).sort();
  const diagnostics: ParseDiagnostics = {
    totalTxnRows,rowsWithBalance,uniqueDates:dateSet.size,
    firstDate:sortedDates[0]||"",lastDate:sortedDates[sortedDates.length-1]||"",
    samplePreview:transactions.slice(0,5).map(t=>`${t.date} | ${t.narration.substring(0,30)} | Dr:${t.debit} Cr:${t.credit} Bal:${t.balance}`),
  };
  if(monthsData.length===0)warnings.push("No valid month buckets found from parsed transactions.");
  if(totalTxnRows===0)warnings.push("No transaction rows detected. PDF may be scanned/image-based.");

  return {
    monthsData,overallABB,totalMonthsFound:monthsData.length,bankName,parseWarnings:warnings,
    diagnostics,detectedPeriod,allTransactions:transactions,
  };
}

export function calculateRiskAssessment(
  result: ClientParseResult, loanMultiplier: number
  ...
  if (monthsData.length >= 2) {
    ...
    if(trendPct>20)score+=15;else if(trendPct>5)score+=8;else if(trendPct<-20)score-=15;else if(trendPct<-5)score-=8;
  }
  ...
  if(abbs.length>=2){
    ...
    if(cv<0.2)score+=15;else if(cv<0.4)score+=8;else if(cv>0.7)score-=10;
  }
  score-=monthsData.reduce((s,m)=>s+m.chequeBounces,0)*10;
  score-=monthsData.reduce((s,m)=>s+m.emiBounces,0)*15;
  const avgCredits=monthsData.reduce((s,m)=>s+m.creditCount,0)/monthsData.length;
  if(avgCredits>=5)score+=10;else if(avgCredits>=2)score+=5;else score-=5;
  if(overallABB>50000)score+=10;else if(overallABB>20000)score+=5;else if(overallABB<5000)score-=5;
  score=Math.max(0,Math.min(100,Math.round(score)));

  let healthGrade: string;
  if(score>=85)healthGrade="A+";else if(score>=70)healthGrade="A";else if(score>=55)healthGrade="B+";else if(score>=40)healthGrade="B";else healthGrade="C";
  ...
  return { riskScore:score, healthGrade, lendingRemarks, suggestedMaxLoan:Math.round(overallABB*loanMultiplier) };
}

export function calculateEMI(
  loanAmount: number, annualRate: number, tenureMonths: number
  ...
  return {loanAmount,interestRate:annualRate,tenure:tenureMonths,monthlyEMI:Math.round(emi),totalInterest:Math.round(totalRepayment-loanAmount),totalRepayment:Math.round(totalRepayment)};
}
'@

Set-Content -Path "src\lib\parseBankClient.ts" -Value $parserContent -Encoding UTF8
Write-Host "        Replaced" -ForegroundColor Green

Write-Host ""
Write-Host "  [3/4] src\lib\generateBankPdf.ts and [4/4] src\pages\BankingSurrogate.tsx" -ForegroundColor White
Write-Host "  These files are too large for PowerShell here-string embedding." -ForegroundColor DarkGray
Write-Host "  They will be written via separate commands." -ForegroundColor DarkGray
Write-Host ""

Write-Host "============================================" -ForegroundColor Green
Write-Host "  Files 1 & 2 created successfully!" -ForegroundColor Green
Write-Host "============================================" -ForegroundColor Green
Write-Host ""
Write-Host "Now run these 2 commands to create the remaining files:" -ForegroundColor Yellow
Write-Host ""
Write-Host "  npx degit mahajan-finance/generateBankPdf src/lib/generateBankPdf.ts" -ForegroundColor White
Write-Host "  (or copy the file content manually)" -ForegroundColor DarkGray
Write-Host ""
